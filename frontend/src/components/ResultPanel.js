...
export default function ResultPanel({ result, preview }) {
  ...
  const subScores = [
    { name: "AI Model", value: Math.round((result?.ai_score || 0) * 100), max: 40, color: "#ff4d6d" },
    { name: "Tampering", value: Math.round((result?.tampering_score || 0) * 100), max: 30, color: "#ff8c42" },
    { name: "Metadata", value: Math.round((result?.metadata_score || 0) * 100), max: 20, color: "#ffd60a" },
    { name: "OCR", value: Math.round((result?.ocr_score || 0) * 100), max: 10, color: "#00d4ff" },
  ];
